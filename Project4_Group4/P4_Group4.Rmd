...
```{r Question2c}
predictNaiveBayes <- function(object,
                               newdata,
                               type = c("class", "raw"),
                               threshold = 0.001,
                               eps = 0,
    ...
    attribs <- match(names(object$condProb), names(newdata))
    ...
    L <- sapply(1:nrow(newdata), function(i) {
        ndata <- newdata[i, ]
        L <- log(object$priorProb) + apply(log(sapply(seq_along(attribs),
            function(v) {
                nd <- ndata[attribs[v]]
                if (is.na(nd)) rep(1, length(object$priorProb)) else {
                  prob <- if (isnumeric[attribs[v]]) {
                    msd <- object$condProb[[v]]
                    msd[, 2][msd[, 2] <= eps] <- threshold
                    dnorm(nd, msd[, 1], msd[, 2])
                  } else object$condProb[[v]][, nd + islogical[attribs[v]]]
...
predictNaiveBayes(nb_fit, test_fruit, type = "class")
...
naiveBayesModel <- function() {
    for(fold in 1:k_fold) {
        ...
        error_rate <- 1 - accuracy
        AUC <- round(AUC(test_prediction, test_set$Type), 2)
        total_acc <- c(total_acc, accuracy)
        total_error_rate <- c(total_error_rate, error_rate)
        total_AUC <- c(total_AUC, AUC)
    }
