...
getNormalized <- function(trData, teData, minV, maxV, method) {
...
getNormalizedDF <- function(trData, teData, minV, maxV, method) {
    trResultList <- list()
    teResultList <- list()
    for(i in 1:ncol(trData)) {
        if(!is.null(teData)) {
            scaled <- getNormalized(trData[, i], teData[, i], minV, maxV, method)
            teResultList[[i]] <- scaled[2]
        } else {
            scaled <- getNormalized(trData[, i], teData, minV, maxV, method)
        }
...
minmaxNorm <- function(trData = NULL, teData = NULL, minV = NULL, maxV = NULL) {
    if(!is.null(trData)) {
        ...
        method <- "minmax"
        if(is.vector(trData)) {
            result <- getNormalized(trData, teData, minV, maxV, method)
        } else if(is.data.frame(trData)) {
            result <- getNormalizedDF(trData, teData, minV, maxV, method)
        } else if(is.matrix(trData)) {
            if(is.null(teData)) {
                result <- getNormalizedDF(as.data.frame(trData), teData, minV, maxV, method)
            } else {
                result <- getNormalizedDF(as.data.frame(trData), 
                                                as.data.frame(teData), minV, maxV, method)
            }
        }
    } else {
