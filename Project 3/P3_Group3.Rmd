...
Hypothesis testing can be done to see if there are significant difference in error rates. Since same data partitioning is used for both M1 and M2 i.e same test set, the paired observation hypothesis testing to compare means is
...
Since p-value is 0.0437 which is greater than the significance level of 0.01, we fail to reject the null hypothesis i.e the two models are not significantly different.
...
**The intuition was facebook but the variable at the root of the tree is Instagram.**
...
**There are 8 terminal leaves with 15 total nodes.**
