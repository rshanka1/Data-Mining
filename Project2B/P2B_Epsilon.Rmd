...
```{r question7 Reading in the data}
...
```{r Feature Scaling for 20 votes}
...
```{r Using ggbiplot for PCA model1}
library(devtools)
...
```{r Using ggbiplot for PCA model2}
library(devtools)
#install_github("ggbiplot","vqv")
library(ggbiplot)
...
```{r}
#readLines(curl(url = "https://api.propublica.org/congress/v1/114/senate/sessions/2/votes/17.json",open = "",handle = "X-API-Key: PROPUBLICA_API_KEY"))

ul <- GET(url = "http://api.propublica.org/congress/v1/104/house/sessions/1/votes.json")
